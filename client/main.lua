									...
									if IsVehicleValid(GetEntityModel(vehicle)) then
										...
										QBCore.Functions.TriggerCallback('qb-scrapyard:checkOwnerVehicle',function(retval)
											if not retval then
												...
												--TriggerServerEvent("qb-scrapyard:deleteOwnerVehicle", vehiclePlate)
											else
												QBCore.Functions.Notify(Lang:t('error.smash_own'), "info")
											end
										end,vehiclePlate)
									else
										QBCore.Functions.Notify(Lang:t('error.cannot_scrap'), "info")
									end
								else
									QBCore.Functions.Notify(Lang:t('error.not_driver'), "info")
								end
							end
						end
					end
...
function CreateListEmail()
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then
		...
		QBCore.Functions.Notify(Lang:t('error.demolish_vehicle'), "info")
	end
end
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", Lang:t('text.demolish_vehicle'), scrapTime, false, true, {
		...
		StopAnimTask(PlayerPedId(), "mp_car_bomb", "car_bomb_mechanic", 1.0)
		isBusy = false
		QBCore.Functions.Notify(Lang:t('error.canceled'), "info")
	end)
end
